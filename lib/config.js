...
async function getGitRev () {
  ...
  let cwd = __filename.indexOf(path.join('build', 'lib', 'config')) !== -1 ?
...
async function getAppiumConfig () {
  ...
  // this code can be run pre-transpiled (so, code is in 'build')
  // or transpiled on the fly (so, code is in root directory)
  let {version} = __filename.indexOf(path.join('build', 'lib', 'config')) !== -1 ?
    require(path.join('..', '..', 'package.json')) : require(path.join('..', 'package.json'));
