...
function getRootDir (filename, buildPath = []) {
  // this code can be run pre-transpiled (so, code is in 'build')
  // or transpiled on the fly (so, code is in root directory)
  return filename.indexOf(path.join(...buildPath)) !== -1 ?
    path.resolve(__dirname, "..", "..") : path.resolve(__dirname, "..");
}

async function getGitRev () {
  let cwd = getRootDir(__filename, ['build', 'lib', 'config']);
  let rev = null;
  try {
...
async function getAppiumConfig () {
  ...
  let cwd = getRootDir(__filename, ['build', 'lib', 'config']);
  let {version} = require(path.join(cwd, 'package.json'));
...
export { getAppiumConfig, validateServerArgs, checkNodeOk, showConfig,
         ...
         getDeprecatedArgs, getGitRev, checkValidPort, getRootDir };
